    ...
    <HeaderContainer>
      <Content>
        ...
        <UserSection>
          ...
          <UserOption>로그인</UserOption>
          <UserOption>회원가입</UserOption>
...
const NavigationMenu = () => (
  <MenuContainer>
    <Menu
      title="영화"
      items={["인기", "현재 상영중", "개봉 예정", "높은 평점"]}
      left="220px"
    />
    <Menu
      title="TV 프로그램"
      items={["인기", "오늘 방영", "TV 방영중", "높은 평점"]}
      left="300px"
    />
    <Menu title="인물" items={["인기 인물"]} left="410px" />
    <Menu
      title="More"
      items={["토론 내역", "기여 랭킹", "지원", "API"]}
      left="480px"
    />
  </MenuContainer>
);

const Menu = ({ title, items, left }) => {
  ...
  return (
    ...
    >
      {title}
      {menuOpen && <Dropdown left={left} items={items} />}
    </MenuItem>
  );
};

const Dropdown = ({ left, items }) => (
  <DropdownList left={left}>
    {items.map((item, index) => (
      <DropdownItem to="/Search" key={index}>
        {item}
      </DropdownItem>
    ))}
  </DropdownList>
...
const LanguageButton = styled.li`
  font-size: 12px;
  padding: 5px;
  border-radius: 5px;
  border: 1px solid #fff;
